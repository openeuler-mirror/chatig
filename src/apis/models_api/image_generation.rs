...
use crate::cores::models::image_generation::image_controller::{ImageGenerationRequest, ImageGenerationResponse, ImageProvider};
use crate::cores::models::image_generation::sdxl::SdxlTurbo;
...
async fn v1_images_generations(req_body: web::Json<ImageGenerationRequest>) -> Result<impl Responder, Error> {
    ...
    if req_body.prompt.is_empty() || req_body.model.is_empty() {
        let error_response = format!("request body is missing required fields: prompt, model");
    ...
    match response {
        ...
        Err(err) => {
            let error_response = format!("Failed to get response from {} image generation: {}", model_name, err);
